...
#xfr = ExtraTreesClassifier(n_estimators =1000,n_jobs=-1,oob_score=True,max_depth=7)
xfr = ExtraTreesClassifier(n_estimators =10000,n_jobs=-1,max_depth=7)
#xfr = RandomForestClassifier (n_estimators =10000,n_jobs=-1,max_depth=7)
print >> log, target_train.shape
xfr = xfr.fit(data_train_nt,[0 if x==0 else 1 for x in target_train])
...
predcted_prob  = pd.concat([data_train,pd.DataFrame(xfr.predict(data_train_nt),columns=['predicted']), pd.DataFrame( xfr.predict_proba(data_train_nt),columns=['proba0','proba1'])],axis=1).sort(['target','proba0'],ascending=[False,False])
...
#log_proba = xfr.predict_log_proba(data_train_nt)


#if 'target_train' in locals():
  #score = xfr.score(data_train_nt,[0 if x is 0 else 1 for x in target_train])
end = time.clock()
